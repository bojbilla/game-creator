...
/**
  * Factory for [[me.reminisce.gameboard.questions.WhichPageDidYouLikeWithDifficulty]]
  */
object WhichPageDidYouLikeWithDifficulty {

  /**
    * Creates a WhichPageDidYouLikeWithDifficulty question generator
    *
    * @param database database from which to take the data
    * @return props for the created actor
    */
  def props(database: DefaultDB): Props =
  Props(new WhichPageDidYouLikeWithDifficulty(database))
}

/**
  * WhichPageDidYouLikeWithDifficulty question generator
  *
  * @param db database from which to take the data
  */
class WhichPageDidYouLikeWithDifficulty(db: DefaultDB) extends QuestionGenerator {
  ...
  def receive = {
              ...
              getDocuments[FBPage](db, pagesCollection, queryNotLiked, 40).onComplete {
                case Success(listPages) =>
                  if (listPages.length < 3) {
                    ...
                    val choices = getChoices(getDifficultyForQuestion(userId), listPages)
    ...
    case any =>
      log.error(s"WhichPageDidYouLikeWithDifficulty received a unexpected message $any")
  }
   ...
   */
  private def getChoices(difficulty: Double, listPages: List[FBPage]): List[FBPage] = {
    val sortedList = listPages.sortBy(-_.likesNumber)
    val pool = Random.shuffle(sortedList.take((-30*difficulty+40).toInt))
    pool.take(3)
  }
}
